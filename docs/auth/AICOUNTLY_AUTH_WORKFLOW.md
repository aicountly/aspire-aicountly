# AICOUNTLY auth workflow (Aspire)

How Aspire signs a user in. This is the shared AICOUNTLY SaaS flow — the same
...
1. User opens `aspire.aicountly.com` (or `aspire.gh.aicountly.com`).
2. No `auth_token` → redirect to
   `{portal}/login/authentication_jump/aspire?returnUrl={origin}/auth/callback`.
...
| Aspire host | Login redirect | Auth API | Product API |
|---|---|---|---|
| `aspire.aicountly.com` | `my.aicountly.com` | `my.aicountly.com` | `aspire.aicountly.com/api` |
| `aspire.gh.aicountly.com` | `sandbox.aicountly.com` | `my.aicountly.com` | `aspire.gh.aicountly.com/api` |
...
`aspire` out of either hostname, and `isSandboxHost()` picks the portal.
...
curl https://aspire.gh.aicountly.com/api/health
...
curl -i -X POST https://aspire.gh.aicountly.com/api/global/seskey

# 3. Unrelayed paths are refused
curl -i -X POST https://aspire.gh.aicountly.com/api/global/login   # expect 404
```

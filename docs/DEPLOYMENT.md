...
| --- | --- | --- | --- |
| Deploy to cPanel Production | `web/dist/` then `server-php/` | `<remote root>/` and `<remote root>/api/` | https://aspire.aicountly.com (+ `/api`) |
| Deploy to cPanel Sandbox | `web/dist/` then `server-php/` | `<remote root>/` and `<remote root>/api/` | https://aspire.gh.aicountly.com (+ `/api`) |
...
`https://aspire.aicountly.com/api/.env` unless Apache is told otherwise.
